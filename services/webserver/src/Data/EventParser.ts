...
export default class EventParser {
    ...
    constructor(inputEvent) {
        ...
        if(!this.error){
            ...
            Log.error(inputEvent.Name + " couldn't be improved, missing fundamental information");
        }
    }
    ...
    improveConfData(data) {
        ...
        if (typeof data.People == 'undefined') {
            Log.error("People element missing (required)");
            return false;
        }
        var people = data.People;
        if (typeof data.People.length == 'undefined') {
            Log.error("People element is not an array");
            return false;
        }
        ...
        if (typeof data.Sessions == 'undefined') {
            Log.error("Sessions element missing (required)");
            return false;
        }
        if (typeof data.Sessions.length == 'undefined') {
            Log.error("Sessions element is not an array (required)");
            return false;
        }
        for (var i = 0; i < data.Sessions.length; i++) {
            ...
            if (typeof session.LocationIndex == 'undefined' || session.LocationIndex == '') { // munge
                if (session.Location == '') {
        ...
        if (typeof data.Items == 'undefined') {
            Log.error("Items element missing (required)");
            return false;
        }
        if (typeof data.Items.length == 'undefined') {
            Log.error("Items element is not an array (required)");
            return false;
        }
